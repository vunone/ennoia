...
def test_parses_openrouter() -> None:
    adapter = parse_llm_spec("openrouter:meta-llama/llama-3.1-8b-instruct")
...
def test_embedding_openrouter() -> None:
    adapter = parse_embedding_spec("openrouter-embedding:openai/text-embedding-3-small")
    assert isinstance(adapter, OpenRouterEmbedding)
    assert adapter.model == "openai/text-embedding-3-small"
